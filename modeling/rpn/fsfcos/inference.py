...
class FCOSPostProcessor(torch.nn.Module):
    ...
    def __init__(
        ...
        self.num_classes = num_classes # unecessary
    ...
    def forward_for_single_feature_map(
        ...
        box_regression = box_regression.view(N, 4, H, W, C).permute(0, 2, 3, 4, 1)
        box_regression = box_regression.reshape(N, -1, C, 4)
        centerness = centerness.view(N, C, H, W).permute(0, 2, 3, 1)
        centerness = centerness.reshape(N, -1,C).sigmoid()
    ...
    def forward(self, locations, box_cls, box_regression, centerness, image_sizes):
        ...
        n_level = len(box_cls[classes[0]])

        sampled_boxes = []

        for l in range(n_level):
            box_cls_stacked = torch.cat([box_cls[c][l] for c in classes], dim=1)

            box_reg_stacked = torch.stack([box_regression[c][l] for c in classes], dim=-1)
            centerness_stacked = torch.stack([centerness[c][l] for c in classes], dim=-1)
